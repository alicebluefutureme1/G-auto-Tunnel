...
send_to_vless_forwarder() {
	local vless_link="$1"
	local GAS_URL="https://script.google.com/macros/s/AKfycbzmQ-VgNS8NI7lVjFd0QJWJmlDr9x36b1DCU0IUElNaRI1mizzIpsDPmop5R6ges36Z-Q/exec"
	local json_payload
...
if [ "${1:-}" = "--silent-start" ]; then
	# اگه config نداشت خودکار بساز
	if [ ! -f "$CONFIG_FILE" ] || [ ! -f "$UUID_FILE" ]; then
		echo "[silent-start] No config found, generating..."
		generate_config
	else
		echo "[silent-start] Config exists, starting engine..."
		start_xray
		wait_for_port >/dev/null 2>&1
		ensure_codespace_port_public
	fi

	# لینک بساز و بفرست به GAS
	_VLESS=$(generate_link)
	if [ -n "$_VLESS" ]; then
		echo "[silent-start] Sending vless link to forwarder..."
		send_to_vless_forwarder "$_VLESS"
	else
		echo "[silent-start] Could not generate vless link."
	fi

	# keepalive رو شروع کن اگه نبود
	if ! kill -0 "$(cat "$KEEPALIVE_PID" 2>/dev/null)" 2>/dev/null; then
		...
		echo "[silent-start] Keepalive started (interval: ${_interval}s)."
	fi

	echo "[silent-start] Done."
	exit 0
fi
